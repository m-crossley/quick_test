	...
	public Gabriel( float[][] points ) {
		...
		for (int i=0; i<faces.length; i++) {
			for (int j=0; j<faces[i].length; j++) {
				...
				if ( p<points.length && q<points.length && !mesh.linked(p, q)) { // && dist(points[p][0], points[q][0], points[p][1], points[q][1]) < maxDistance){
					//println("("+ points[p][0] +", " + points[p][1] + ") - (" + points[q][0] + ", " + points[q][1] + ")");
					boolean connect = true;
					float distBetweenPQ = PApplet.dist(points[p][0], points[p][1], points[q][0], points[q][1]);
					for (int k = 0; k < points.length; k++)
					{
						if (k != p && k != q)
						{
							float distBetweenPK = PApplet.dist(points[p][0], points[p][1], points[k][0], points[k][1]);
							float distBetweenQK = PApplet.dist(points[q][0], points[q][1], points[k][0], points[k][1]);
							//println("PQ: " + distBetweenPQ + ", PK: " + distBetweenPK + ", QK: " + distBetweenQK);
							if (Math.pow(distBetweenPQ, 2) > Math.pow(distBetweenPK, 2)+Math.pow(distBetweenQK, 2))
							{
								connect = false;
							}
						}
